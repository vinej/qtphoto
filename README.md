...
`Entrée` appelle `/usr/bin/jyvux-gimp <photo>`, `Ctrl+Entrée`
`/usr/bin/jyvux-krita <photo>`, s'ils existent — sur JYVUX, les deux vivent
dans des conteneurs podman et ces lanceurs savent les joindre, même déjà
ouverts. `Shift+Entrée` appelle `nomacs <photo>` (natif). Ailleurs :
`gimp` / `krita` / `nomacs` dans le `PATH`.
